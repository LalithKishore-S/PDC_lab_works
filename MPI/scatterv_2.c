...
int main(int argc,char ** argv){
	...
	if(r==0){
		...
		send = (int *)malloc(n*(n+1)/2*sizeof(int));
	...
	MPI_Scatterv(send,send_count,dis,MPI_INT,recv,send_count[r],MPI_INT,0,MPI_COMM_WORLD);


	for(int i=0;i<send_count[r];i++) recv[i] *= 10;

	MPI_Gatherv(recv,send_count[r],MPI_INT,send,send_count,dis,MPI_INT,0,MPI_COMM_WORLD);
